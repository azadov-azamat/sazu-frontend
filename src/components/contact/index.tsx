...
export default function Component() {
    ...
    return (
        <section id={'contacts'}>
            <PageTitleComponent title={t('about-team')} className={'md:justify-center justify-start'}/>
            <div className={'w-full'}>
                {selectedProfile && (
                    <motion.div
                        initial={{opacity: 0, y: -200}}
                        animate={{opacity: 1, y: 0}}
                        className="md:w-[700px] w-full xl:h-[490px] md:h-[350px] h-auto flex sm:flex-row flex-col items-center gap-8 sm:p-8 p-0
                         mx-auto md:mt-8 mt-2"
                    >
                        <LazyLoadImage
                            loading={'lazy'}
                            src={selectedProfile.image}
                            alt={selectedProfile.name}
                            className="w-full sm:w-1/2 md:w-96 h-full object-cover object-center rounded-lg mb-4 transition-transform duration-500 ease-in-out hover:scale-95"
                        />
            ...
            <motion.div
                className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 xl:grid-cols-4 sm:gap-10 md:mt-8 mt-4"
                initial={{opacity: 0}}
                animate={{opacity: 1}}
                transition={{delay: 1}}
            >
